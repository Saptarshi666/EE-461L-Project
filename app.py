...
def get_availability():
        ...
        print(params)
        setName, availability = params['HWSet Name'].strip(), params['Availability'].strip()
        print(type(setName))
        db = Client['HWSets']
        set = db[setName]
...
def check_out_hw(): # (projID, setName, number):
        ...
        db = Client['Projects']
        proj = db[projID]
        proj.find_one_and_update({"_id": projID}, {"$inc":{setName: number}})
        return jsonify(result = "True")
...
def check_in_hw(): # (setName, number):
        ...
        db = Client['Projects']
        proj = db[projID]
        # checks proj for post about hwset
